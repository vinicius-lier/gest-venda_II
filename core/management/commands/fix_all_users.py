...
class Command(BaseCommand):
    help = 'Verifica e corrige todos os usuários do sistema'

    def handle(self, *args, **options):
        self.stdout.write('Verificando e corrigindo usuários do sistema...')
        ...
        try:
            with transaction.atomic():
                ...
                perfil_gerente, created = Perfil.objects.get_or_create(
                    nome='gerente',
                    defaults={'descricao': 'Gerente do sistema'}
                )
                
                perfil_vendedor, created = Perfil.objects.get_or_create(
                    nome='vendedor',
                    defaults={'descricao': 'Vendedor do sistema'}
                )
                
                perfil_consultor, created = Perfil.objects.get_or_create(
                    nome='consultor',
                    defaults={'descricao': 'Consultor do sistema'}
                )
                    ...
                    try:
                        ...
                        if created:
                            ...
                            if usuario_sistema.loja != loja_padrao:
                                ...
                                self.stdout.write(f'Usuario já configurado: {user.username}')
                                
                    except Exception as e:
                        self.stdout.write(
                            self.style.ERROR(f'Erro ao configurar {user.username}: {str(e)}')
                        )
            ...
            logger.error(f'Erro no comando fix_all_users: {str(e)}')
            raise 